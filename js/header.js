$(document).ready(function() {
	...
	$menuItem.on('click', function(e) {
		...
		if ($submenu.length) {
			if ($(this).children('.header-nav-submenu').css('display') == 'none') {
				flag = false;
			} else {
				flag = true;
			}
			if (flag == true) {
				$(this).children('.header-nav-submenu').css('display', 'none');
				flag = false;
			} else {
				$(this).children('.header-nav-submenu').css('display', 'block');
				var item = $(this).get(0).parentNode.children;
				for (var i = 0; i < item.length; i++) {
					if (item[i] != $(this).get(0)) {
						$(item[i]).children('.header-nav-submenu').css('display', 'none');
					}
				}
				flag = true;
			}
